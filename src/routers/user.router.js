...
router.post('/', newUserValidation, async (req, res) => {
  ...
  console.log(req.body, 'REQUEST BODY REGISTER +++++++++++++++++++++');
  ...
  try {
    ...
    console.log(err);
    let message =
      'Unable to create new user at the moment. Pleaset contact administrator';
    ...
    if (email) {
      await updateUser(_id.toString(), newData)
        .then(async (data) => {
          ...
          res.json({
            status: 'success',
            message: 'Sended again. Check your email for a verification link',
            result,
          });
        })
        .catch((error) => {
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    if (!user.isVerified)
      return res.json({
        status: 'Not Verified',
        message: 'Waiting for user to validate email address',
      });
...
router.post('/google-registration', async (req, res) => {
  try {
    ...
    if (decoded.iss != '') {
      ...
      res.json({
        status: 'success',
        message: 'New user created from Google Accounts',
        result,
      });
    }
  } catch (err) {
...
router.post('/google-login', async (req, res) => {
  try {
    await decodeGoogleJWT(req.body.credential)
      .then(async (decoded) => {
...
router.post('/reset-password', resetPassReqValidation, async (req, res) => {
  ...
  try {
    ...
    if (user && user._id) {
      ...
      const recoveryUrl =
        process.env.RECOVERY_URL + '/' + setPin.pin + '/' + email;
      //5 - email the pìn
      ...
      if (result && result.messageId) {
        return res.json({
          status: 'success',
          message:
            'If the email exists in our database, the password reset pin will be send shortly',
        });
      }
      return res.json({
        status: 'success',
        message:
          'Email found. A recovery PIN is being sent to you. Check your email and follow instructions',
        setPin,
      });
    }
  } catch (error) {
    res.json({
      status: 'error',
      message:
        'Unable to process your request at the moment - Try again later. If the email exists in our databes, the password reset pin will be send shortly',
    });
  }
});
...
router.post('/valoration', async (req, res) => {
  ...
  try {
    ...
    console.log(err);
    let message =
      'Unable to create Valoration at the moment. Pleaset contact administrator';
    res.json({ status: 'error', message });
  }
});
...
router.patch('/valorationMod', async (req, res) => {
  ...
  try {
    const result = await acceptRejectUserValoration(
      accepted,
      rejected,
      user_id,
      val_id
